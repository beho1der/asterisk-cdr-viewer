...
/// заменяем рукописный ввод даты на jquery datapicket + timepicker
$time_start=htmlspecialchars(trim($_REQUEST['time_start']));
$time_end=htmlspecialchars(trim($_REQUEST['time_end']));
/**
$startmonth = is_blank($_REQUEST['startmonth']) ? date('m') : $_REQUEST['startmonth']; # если не заполнено берем текущий месяц
$startyear = is_blank($_REQUEST['startyear']) ? date('Y') : $_REQUEST['startyear']; # если не заполнено берем текущий год


if (is_blank($_REQUEST['startday'])) {
	$startday = '01';
} elseif (isset($_REQUEST['startday']) && ($_REQUEST['startday'] > date('t', strtotime("$startyear-$startmonth")))) { #проверка если число больше чем колл-вл дней в месяце,то ставим последний день месяца
	$startday = $_REQUEST['startday'] = date('t', strtotime("$startyear-$startmonth"));
} else {
	$startday = sprintf('%02d',$_REQUEST['startday']); #ставится 0 перед днем
}
...
$start_timestamp = mktime( $starthour, $startmin, 59, $startmonth, $startday, $startyear ); # собирает начальное  время в unix 59 это секунды
...
*/
$startday =   substr_replace($time_start,'',2) ;
$startmonth = substr($time_start,3,-11) ;
$startyear =  substr($time_start,6,-6) ;
$starthour =  substr($time_start,11,-3) ;
$startmin  =  substr_replace($time_start,'',0,-2);
//
$endday =   substr_replace($time_end,'',2) ;
$endmonth = substr($time_end,3,-11) ;
$endyear =  substr($time_end,6,-6) ;
$endhour =  substr($time_end,11,-3) ;
$endmin  =  substr_replace($time_end,'',0,-2);
//
$startdate = "'$startyear-$startmonth-$startday $starthour:$startmin:00'";
$start_timestamp = mktime( $starthour, $startmin, 59, $startmonth, $startday, $startyear ); # собирает начальное  время в unix 59 это секунды
$enddate = "'$endyear-$endmonth-$endday $endhour:$endmin:59'";
$end_timestamp = mktime( $endhour, $endmin, 59, $endmonth, $endday, $endyear ); # собирает конечное  время в unix 59 это секунды
...
if ( $tot_calls_raw ) {
	echo '<p class="center title">Call Detail Record - Search Returned '. $tot_calls_raw .'Calls </p><table class="tablesorter" id="myTable" >';
		$i = $h_step - 1;
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		?>
			...
			if ( isset($_REQUEST['use_callrates']) && $_REQUEST['use_callrates'] == 'true' ) {
				echo '<th>CallRate</th><th>CallRate Dst</th>';
			}
			?>
			</tr>
			</thead>

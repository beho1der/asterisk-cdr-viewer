...
$(function() {
  ...
  $(document).bind('click', function(e) {
	...
	$('select.styled').each(function() {
		...
		/* при клике на псевдоселекте(event on click span select) */
		$(this).prev().find('span.select').click(function() {

			/* умное позиционирование(clever positioning) */
			var topOffset = $(this).parent().offset().top;
			var bottomOffset = $(window).height() - selectHeight - (topOffset - $(window).scrollTop());
			if (bottomOffset < 0 || bottomOffset < liHeight * 4)	{
			...
			} else if (bottomOffset > liHeight * 4) {
		...
		/* при наведении курсора на пункт списка(event hover over a list item) */
		$(this).prev().find('li:not(.disabled)').hover(function() {
			$(this).siblings().removeClass('selected');
		})
		/* при клике на пункт списка (event click on the item list) */
		.click(function() {
		...
		/* фокус на селекте при нажатии на Tab( focus on Select on click key "Tab") */
		$(this).focus(function() {
			$('span.selectbox').removeClass('focused');
			$(this).prev().addClass('focused');
		})
		/* меняем селект с клавиатуры(change the SELECT of keypad) */
		.keyup(function() {

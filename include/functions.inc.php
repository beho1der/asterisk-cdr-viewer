...
function formatFiles($row) {
	...
	*/
	...
	if (file_exists("$system_monitor_dir/$recorded_file.$system_audio_format")) {
		echo "    <td><a href=\"download.php?audio=$recorded_file.$system_audio_format\" title=\"Listen to call recording\"><img src=\"/icons/small/sound.png\" alt=\"Call recording\" /></a></td>\n";
	} elseif (file_exists("$system_fax_archive_dir/$recorded_file.tif")) {
		echo "    <td><a href=\"download.php?fax=$recorded_file.tif\" title=\"View FAX image\"><img src=\"/icons/small/text.png\" alt=\"FAX image\" /></a></td>\n";
	} 
/*	else {
		echo "    <td></td>\n";
	} */
...
function formatCallDate($calldate,$uniqueid) {
	echo "    <td><abbr title=\"UniqueID: $uniqueid\">$calldate</abbr></td>\n";
}

function formatChannel($channel) {
	$chan_type = explode('/', $channel, 2);
	echo "    <td><abbr title=\"Channel: $channel\">$chan_type[0]</abbr></td>\n";
}
...
function formatApp($app, $lastdata) {
	echo "    <td><abbr title=\"Application: $app($lastdata)\">$app</abbr></td>\n";
}
...
function formatDst($dst, $dcontext) {
	...
	if (strlen($dst) == 11 and strlen($rev_lookup_url) > 0 ) {
		...
		echo "    <td><abbr title=\"Destination Context: $dcontext\"><a href=\"$rev\" target=\"reverse\">$dst</a></abbr></td>\n";
	} else {
		echo "    <td><abbr title=\"Destination Context: $dcontext\">$dst</abbr></td>\n";
	}
}
...
function formatDisposition($disposition, $amaflags) {
	...
	echo "    <td><abbr title=\"AMA Flag: $amaflags\">$disposition</abbr></td>\n";
}
...
function formatDuration($duration, $billsec) {
	...
	echo "    <td><abbr title=\"Billing Duration: $billduration\">$duration</abbr></td>\n";
}
...
function formatMoney($number, $cents = 2) { // cents: 0=never, 1=if needed, 2=always
	...
	if (is_numeric($number)) { // a number
		...
		echo   "<td>$callrate_currency<span>$money</span></td>\n";
	} else {
		echo   "<td>&nbsp;</td>\n";
	}
} // formatMoney
